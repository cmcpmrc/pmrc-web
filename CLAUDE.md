...
### 교육정보 - 초청 세미나
- seminar 폴더(C:\pmrc-web\seminar)에 PDF만 유지한다 (이미지 변환 없음).
- 새 세미나 추가 시 PDF에서 제목·발표자·날짜·장소만 텍스트로 추출해 카드를 구성한다.
  **PDF 파일이나 그 안의 이미지를 education.html에 링크·삽입하지 않는다** — 텍스트 정보만 보여준다.
- 발표자 개인 사진이 포함된 이미지 파일은 pmrc-web에 두지 않고 저장소에도 커밋하지 않는다.
- 기존에 seminar 폴더/education.html에 있던 포스터 썸네일 이미지·발표자 사진·PDF 링크는
  모두 제거 완료 (2026-08-06, 과거 커밋 기록도 filter-repo로 삭제).
...
education.html  교육정보     — 분석 워크샵 3종 / 교육 워크샵 목록(표) /
                              초청세미나 (공고문 PDF에서 추출한 텍스트 정보만 표시)
...
seminar/               초청세미나 공고문 pdf만 보관한다 (현장 사진·포스터 이미지는 없음).
                       education.html에서 링크하지 않는다 — 카드에는 pdf에서 추출한
                       텍스트(제목·발표자·날짜·장소)만 싣는다. Pages가 저장소 전체를
                       서빙하므로 URL을 알면 직접 접근은 가능하며, 공개해도 된다는
                       확인을 받았다(2026-07-27).
...
`seminar/<YYYYMMDD>.pdf`(공고문) 하나만 저장소에 넣으면 된다. **이미지 변환도,
사이트에 pdf를 링크하는 것도 하지 않는다** — pdf 안의 제목·발표자·날짜·장소 정보만
텍스트로 뽑아서 education.html `#seminars`의 `<article>`을 최신이 위에 오도록 하나
복제해 채운다. 카드는 `<time>`/제목(`<h3>`)/발표자·장소(`<p>`) 텍스트뿐이고, pdf나
이미지로 이어지는 링크는 넣지 않는다.

**포스터 썸네일·현장 사진·PDF 링크 모두 사이트에 올리지 않는다** (2026-08-06 제거,
과거 커밋 기록도 filter-repo로 삭제). 공고문 포스터에 발표자 개인 사진이 들어있는
경우가 많아, 텍스트 정보만 노출하고 이미지·원문 링크는 사이트에서 빼기로 했다. 현장
사진 원본이 있어도 seminar/ 에 커밋하지 말 것 — 필요하면 `pmrc-data` 쪽에 둔다.

발표자명·제목은 파일명(날짜)만으로는 알 수 없다. pdf 본문에서 읽거나 `pmrc-data`에
관련 비공개 메모가 있으면 그걸 참고하고, 정보를 확인할 수 없으면 지어내지 말고
날짜만으로 카드를 구성한다.
...
- 연락처는 `cmcpmrc@gmail.com` / `02-3147-9008` /
  서울시 서초구 반포대로 222 가톨릭대학교 의과대학 의생명산업연구원 3017-2 가 정확한 값이다.
